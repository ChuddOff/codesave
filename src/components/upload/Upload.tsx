...
const Upload = () => {
  ...
  useEffect(() => {
    ...
    const onmousemoveY = (resizer: ExtendedDiv) => (e: MouseEvent) => {
      ...
      if (deltaY < 0) {
        ...
        if (top.className == "div2") {
          ...
          firstDivRef.current!.style.flex = `0 ${
            firstDivRef.current!.clientHeight
          }px`;
      ...
      if (deltaY > 0) {
        ...
        if (bottom.className == "div2") {
          ...
          top.style.flex = "1 0";
          secondDivRef.current!.style.flex = `0 1 ${
    ...
    return () => {
      ...
      if (resizer3 !== null) {
        resizer3.removeEventListener("mousedown", onmousedownX(resizer3));
      }
    };
  }, []);
